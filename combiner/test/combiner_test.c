...
int test_hybrid(combiner_t combiner, size_t hybrid_len, scheme_t* schemes) {
    ...
    key_pair_t key_pair = {
        .public_key = malloc(hybrid_len * sizeof(unsigned char*)), 
        .secret_key = malloc(hybrid_len * sizeof(unsigned char*))
    };

    int ret;
    ret = combiner_keygen(hybrid, &key_pair);
    if (ret != 0) {
    ...
    ret = combiner_sign(&hybrid, key_pair.secret_key,
                        message);
    if (ret != 0) {
    ...
    bool sig_pass = combiner_verify(hybrid, key_pair.public_key,
                                    message);
    if (!sig_pass) {
